...
import math
import joblib
sys.path.append(os.path.join('..', '쇼핑광고등어'))
...
sale = pd.read_excel(os.path.join('..', 'data', '01_제공데이터', '2020 빅콘테스트 데이터분석분야-챔피언리그_2019년 실적데이터_v1_200818.xlsx'), skiprows = 1)
...
data = joblib.load(os.path.join('..', 'data', '04_임시데이터', 'recommend_candidate.pkl'))
locals().update(data)
...
res.to_excel(os.path.join('..', 'data', '01_제공데이터', 'data4recommend.xlsx'), index = False)
...
if args.dataset == 'train':
    ...
    timeS = joblib.load(os.path.join('..', 'data', '04_임시데이터', 'data4time.pkl'))
    ...
    }, os.path.join('..', 'data', '04_임시데이터', 'time4wnd.pkl'))
    ...
    joblib.dump({
        ...
        'volume_v3' : count,
        'volume_v4' : hour
    }, os.path.join('..', 'data', '04_임시데이터', 'volume4wnd.pkl'))
    print('>>>>>>>>>> Saved Volume FE preprocessing for Recommend!! <<<<<<<<<<')


    ## ======== 시청률 Feature 생성 ========
    print('========== Start Rating FE preprocessing for Recommend==========')
    rate = pd.read_excel(os.path.join('..', 'data', '01_제공데이터', '2020 빅콘테스트 데이터분석분야-챔피언리그_시청률 데이터.xlsx'), skiprows = 1)
    ...
    joblib.dump({
        ...
        'rate_v7' : rate_v7,
        'rate_v8' : rate_v8,
    }, os.path.join('..', 'data', '04_임시데이터', 'rate4wnd.pkl'))
...
def preprocessing(df):
    ...
    # 날씨 데이터 FE  
    w_19 = pd.read_csv(os.path.join('..', 'data', '03_외부데이터', '2019_weather.csv'), encoding = 'cp949', dtype='unicode')
    w_20 = pd.read_csv(os.path.join('..', 'data', '03_외부데이터', '2020_weather.csv'), encoding = 'cp949', dtype='unicode')
    df_wth = pd.concat([w_19, w_20], axis = 0)
    df_wth = FEex.preprocessing_weather(df_wth)

    # 미세먼지 데이터
    dust_2019 = pd.read_csv(os.path.join('..', 'data', '03_외부데이터', '2019_dust.csv'), encoding = 'cp949')
    dust_2020 = pd.read_csv(os.path.join('..', 'data', '03_외부데이터', '2020_dust.csv'), encoding = 'cp949')
...
if args.dataset == 'train':
    train = joblib.load(os.path.join('..', 'data', '04_임시데이터', 'train_data4WnD.pkl'))
    test = joblib.load(os.path.join('..', 'data', '04_임시데이터', 'test_data4WnD.pkl'))
    
elif args.dataset == 'test':
    data = pd.read_excel(os.path.join('..', 'data', '01_제공데이터', '2020 빅콘테스트 데이터분석분야-챔피언리그_방송편성표추천데이터.xlsx'))
...
# 내부데이터
volume4wnd = joblib.load(os.path.join('..', 'data', '04_임시데이터', 'volume4wnd.pkl'))
locals().update(volume4wnd)
time4wnd = joblib.load(os.path.join('..', 'data', '04_임시데이터', 'time4wnd.pkl'))
locals().update(time4wnd)
rate4wnd = joblib.load(os.path.join('..', 'data', '04_임시데이터', 'rate4wnd.pkl'))
...
if args.dataset == 'train':
    ...
    }, os.path.join('..', 'data', '05_분석데이터', 'train_Rec.pkl'))
    ...
    joblib.dump({
        'X' : X,
        'data4test' : data4test,
    }, os.path.join('..', 'data', '05_분석데이터', 'test_Rec.pkl'))
