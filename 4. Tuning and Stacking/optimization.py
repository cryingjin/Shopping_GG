...
def xgb_model(X, y, params, cv_splits=5, scaling=False, epoch=10000):        
    ...
    pred = {'val_idx'  : [], 'val_pred'  : [],
            'test_idx' : [], 'test_pred' : [],
            'final_pred' : []}      # final : test set mean ê°’
    ...
    for t,v in cv.split(X_train_):
        ...
        test_T = xgb.DMatrix(X_test, missing=np.nan)
    ...
    final_mape = MAPE(np.expm1(y_test_), final_test)
...
# LightGBM
def lgbm_model(X, y, params, cv_splits=5, scaling=False, epoch=10000):        
    mape = {'val_mape' : [], 'test_mape' : [], 'final_mape' : []}
    pred = {'val_idx'  : [], 'val_pred'  : [],
            'test_idx' : [], 'test_pred' : [],
            'final_pred' : []}      # final : test set mean ê°’
