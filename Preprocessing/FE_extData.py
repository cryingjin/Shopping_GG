...
def preprocessing_economy():
    df1 = pd.read_excel(os.path.join( '..', 'data', '03_외부데이터', '소매업태별 판매액지수.xlsx')) # 출처 : KOSIS
    df2 = pd.read_csv(os.path.join('..', 'data', '03_외부데이터', '소비자동향조사 전국.csv'), encoding = 'cp949') # 출처 : KOSIS
    df3 = pd.read_excel(os.path.join('..', 'data', '03_외부데이터', '온라인쇼핑몰 판매매체별 상품군별거래액.xlsx')) # 출처 : KOSIS
    df4 = pd.read_excel(os.path.join('..', 'data', '03_외부데이터', '지역별 소비유형별 개인 신용카드.xlsx')) # 출처 : 한국은행
