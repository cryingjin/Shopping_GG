...
if args.dataset == 'train':
    ...
    sale = pd.read_excel(os.path.join('..','data','02_평가데이터', '2020 빅콘테스트 데이터분석분야-챔피언리그_2020년 6월 판매실적예측데이터(평가데이터).xlsx'), skiprows = 1)
    test_index = sale.index
elif args.dataset == 'recommend':
    sale = pd.read_excel(os.path.join('..','data','01_제공데이터', 'data4recommend.xlsx'))
else:
    print('dataset error.....')
...
# 날씨 데이터 FE
w_19 = pd.read_csv(os.path.join('..','data','03_외부데이터', '2019_weather.csv'), encoding = 'cp949', dtype='unicode')
w_20 = pd.read_csv(os.path.join('..','data','03_외부데이터', '2020_weather.csv'), encoding = 'cp949', dtype='unicode')
df_wth = pd.concat([w_19, w_20], axis = 0)
df_wth = FEex.preprocessing_weather(df_wth)

# 미세먼지 데이터 FE
if args.dataset == 'train' or args.dataset == 'recommend':
    dust_2019 = pd.read_csv(os.path.join('..','data','03_외부데이터', '2019_dust.csv'), encoding = 'cp949')
    dust_2020 = pd.read_csv(os.path.join('..','data','03_외부데이터', '2020_dust.csv'), encoding = 'cp949')
    df_dust = pd.concat([dust_2019, dust_2020], axis = 0)
    df_dust = FEex.preprocessing_dust(df_dust, args.dataset)
elif args.dataset == 'test':
    df_dust = pd.read_excel(os.path.join('..','data','03_외부데이터', '2020_dust', '2020년 6월.xlsx'))
...
if args.dataset == 'train':
    ...
    joblib.dump({
        ...
        os.path.join('..','data','05_분석데이터', 'train_data4WnD.pkl'))
    ...
    joblib.dump({
        'X' : X,
        'y' : y
    }, os.path.join('..','data','05_분석데이터', 'train_FE.pkl'))
    ...
    joblib.dump({
        ...
        os.path.join('..','data','05_분석데이터', 'test_data4WnD.pkl'))
    
    X = pd.get_dummies(data)
    
    train = joblib.load(os.path.join('..','data','05_분석데이터', 'train_FE.pkl'))
    ...
    if args.dataset == 'test':
        ...
        joblib.dump({
            'X' : X,
            'idx' : test_index
        }, os.path.join('..','data','05_분석데이터', 'test_FE.pkl'))
        ...
        joblib.dump({
            'X' : X,
            'idx' : test_index
        }, os.path.join('..','data','05_분석데이터', 'Rec_FE.pkl'))
